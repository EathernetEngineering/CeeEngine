/* Implementation of old renderer 2D specialisation.
...
namespace cee {
	...
	std::shared_ptr<oldRenderer> Renderer2D::s_Renderer = NULL;
	...
	void Renderer2D::Init(MessageBus* msgBus, std::shared_ptr<Window> window) {
		...
		s_Renderer = std::make_shared<oldRenderer>(rendererCapabilities, window);
