/* Implementation of old renderer.
...
namespace cee {
	VkFormat CeeFormatToVkFormat(::cee::Format foramt) {
		switch (foramt) {
			...
			case FORMAT_DEPTH:
				return oldRenderer::Get()->GetDepthFormat();
			default:
				return VK_FORMAT_UNDEFINED;
		}
	}

	VkBool32 oldRenderer::VulkanDebugMessengerCallback(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity,
		...
		char message[4096];
		ErrorSeverity ceeMessageSeverity;
	...
	: m_Pool(VK_NULL_HANDLE), m_Layout(VK_NULL_HANDLE)
	{
	}
	...
	DescriptorManager::DescriptorManager(DescriptorManager&& other) {

	}

	DescriptorManager& DescriptorManager::operator=(DescriptorManager&& other) {


		return *this;
	}

	oldRenderer* oldRenderer::s_Instance = NULL;

	oldRenderer::oldRenderer(const RendererCapabilities& capabilities,
	...
	int oldRenderer::Init()
	{
...
#endif
		{
			...
			DebugMessenger::PostDebugMessage(ERROR_SEVERITY_DEBUG, "physical device properties:\n"
											"\tDevice Name: %s\n"
											"\tVendor ID: %u\n"
											"\tDiscrete: %s\n"
											"\tAPI Version: %u.%u.%u\n",
...
*/
	...
	void oldRenderer::Clear(const glm::vec4& clearColor) {
		m_ClearColor = clearColor;
	}

	int oldRenderer::StartFrame() {
		ZoneScoped;
		VkResult result;
...
retryAqurireNextImage:
	...
	int oldRenderer::Draw(IndexBuffer& indexBuffer, VertexBuffer& vertexBuffer, uint32_t indexCount) {
		ZoneScoped;
	...
	int oldRenderer::UpdateCamera(Camera& camera) {
		ZoneScoped;
	...
	void oldRenderer::UpdateSkybox(CubeMapBuffer& newSkybox) {
	...
	void oldRenderer::InvalidateSwapchain() {
		ZoneScoped;
	...
	void oldRenderer::InvalidatePipeline() {

	}

	VkResult oldRenderer::ImmediateSubmit(std::function<void(VkCommandBuffer&)> fn, CommandQueueType queueType) {
		ZoneScoped;
		VkCommandBufferAllocateInfo commandBufferAllocateInfo = {};
	...
	VkResult oldRenderer::QueueSubmit(std::function<void(VkCommandBuffer&)> fn, CommandQueueType queueType) {
		ZoneScoped;
		VkResult result = VK_SUCCESS;
	...
	VkResult oldRenderer::FlushQueuedSubmits() {
		ZoneScoped;
		VkResult result = VK_SUCCESS;
	...
	VkCommandBuffer oldRenderer::StartCommandBuffer(CommandQueueType queueType) {
		ZoneScoped;
		VkCommandBufferAllocateInfo commandBufferAllocateInfo = {};
		commandBufferAllocateInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
		commandBufferAllocateInfo.pNext = NULL;
	...
	VkResult oldRenderer::SubmitCommandBufferNow(VkCommandBuffer commandBuffer, CommandQueueType queueType) {
		ZoneScoped;
		VkResult result = VK_SUCCESS;
	...
	void oldRenderer::QueueCommandBuffer(VkCommandBuffer commandBuffer, CommandQueueType queueType) {
		ZoneScoped;
		VkResult result;
	...
	VkPhysicalDevice oldRenderer::ChoosePhysicalDevice(uint32_t physicalDeviceCount, VkPhysicalDevice* physicalDevices)
	{
	...
	VkSurfaceFormatKHR oldRenderer::ChooseSwapchainSurfaceFormat(std::vector<VkSurfaceFormatKHR> availableSurfaceFormats) {
	...
	VkPresentModeKHR oldRenderer::ChooseSwapchainPresentMode(std::vector<VkPresentModeKHR> availablePresentModes) {
	...
	VkExtent2D oldRenderer::ChooseSwapchainExtent(const VkSurfaceCapabilitiesKHR& surfaceCapabilities,
											   std::shared_ptr<Window> window) {
	...
	uint32_t oldRenderer::ChooseMemoryType(uint32_t typeFilter,
	...
	VkShaderModule oldRenderer::CreateShaderModule(VkDevice device, std::vector<uint8_t> shaderCode) {
	...
	std::vector<uint8_t> oldRenderer::AttemptPipelineCacheRead(const std::string& filePath) {
	...
	void oldRenderer::PipelineCacheWrite(const std::string& filePath, const std::vector<uint8_t>& cacheData) {
	...
	VkFormat oldRenderer::ChooseDepthFormat(VkPhysicalDevice physicalDevice, const std::vector<VkFormat>& candidates, VkImageTiling tilingMode, VkFormatFeatureFlags features) {
	...
	VkResult oldRenderer::CreateImageObjects(VkImage* image, VkDeviceMemory* deviceMemory, VkImageView* imageView,
	...
	int oldRenderer::TransitionImageLayout(VkImage image,
	...
	VkResult oldRenderer::CreateBufferObjects(VkBuffer* buffer, VkDeviceMemory* deviceMemory, size_t* size,
										   ...
										   VkMemoryPropertyFlags optimalMemoryPropertyFlags, bool* optimalMemory)
	{
		...
		VkMemoryAllocateInfo bufferAllocateInfo = {
			...
			.memoryTypeIndex = oldRenderer::ChooseMemoryType(bufferMemoryRequirements.memoryTypeBits,
														  oldRenderer::Get()->GetDeviceMemoryProperties(),
	...
	ImageBuffer oldRenderer::CreateImageBuffer(size_t width, size_t height, Format format) {
		ImageBuffer buffer;
		...
		if (format == FORMAT_DEPTH) {
			result = oldRenderer::Get()->CreateImageObjects(&buffer.m_Image,
